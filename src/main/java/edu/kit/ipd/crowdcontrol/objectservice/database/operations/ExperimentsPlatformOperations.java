...
public class ExperimentsPlatformOperations extends AbstractOperations {
    ...
    /**
     * Searches for a task specified by the experimentsPlatformId.
     * @param experimentsPlatformId the primary key of the experiments-platform
     * @return the found task or empty if not found
     */
    public Optional<ExperimentsPlatformRecord> getExperimentsPlatform(int experimentsPlatformId) {
        return create.selectFrom(EXPERIMENTS_PLATFORM)
                .where(EXPERIMENTS_PLATFORM.IDEXPERIMENTS_PLATFORMS.eq(experimentsPlatformId))
                .fetchOptional();
    }
        ...
        return create.select(EXPERIMENTS_PLATFORM.PLATFORM, mode1.MODE)
                ...
                .leftOuterJoin(mode2).on(
                        ...
                        .and(mode1.TIMESTAMP.lessThan(mode2.TIMESTAMP).or(mode1.TIMESTAMP.eq(mode2.TIMESTAMP)
                                .and(mode1.ID_EXPERIMENTS_PLATFORM_MODE.lessThan(mode2.ID_EXPERIMENTS_PLATFORM_MODE))))
                )
                .where(mode2.ID_EXPERIMENTS_PLATFORM_MODE.isNull())
                .and(EXPERIMENTS_PLATFORM.EXPERIMENT.eq(experimentId))
     ...
     * @return a map where the key are the Experiments-Platform IDs and the value the statuses
     */
    public Map<Integer, ExperimentsPlatformStatusPlatformStatus> getExperimentsPlatformStatusPlatformStatuses (int experiment) {
        ...
        return create.select(EXPERIMENTS_PLATFORM.IDEXPERIMENTS_PLATFORMS, status1.PLATFORM_STATUS)
                ...
                .and(EXPERIMENTS_PLATFORM.EXPERIMENT.eq(experiment))
                .fetchMap(EXPERIMENTS_PLATFORM.IDEXPERIMENTS_PLATFORMS, status1.PLATFORM_STATUS);
    ...
    public List<ExperimentRecord> getExperimentsFailedDuringShutdown() {
        ...
        Set<Integer> experimentsPlatforms = create.select(EXPERIMENTS_PLATFORM.IDEXPERIMENTS_PLATFORMS, status1.PLATFORM_STATUS)
                ...
                .where(status2.ID_EXPERIMENTS_PLATFORM_STATUS.isNull())
                .having(status1.PLATFORM_STATUS.in(Arrays.asList(
                        ExperimentsPlatformStatusPlatformStatus.shutdown,
                        ExperimentsPlatformStatusPlatformStatus.shutdown_failed
                )))
