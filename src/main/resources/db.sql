...
CREATE TABLE IF NOT EXISTS `crowdcontrol`.`Experiments_Platform` (
  ...
  UNIQUE INDEX `platform_UNIQUE` (`platform` ASC),
...
CREATE TABLE IF NOT EXISTS `crowdcontrol`.`Task` (
  ...
  `experiment_platform` INT(11) NOT NULL,
  `platform_data` MEDIUMTEXT NULL DEFAULT NULL,
  `stopgap` ENUM('answer', 'rating', 'disabled') NOT NULL DEFAULT 'disabled',
  PRIMARY KEY (`id_task`),
  INDEX `task_platform_idx` (`experiment_platform` ASC),
  CONSTRAINT `task_platform`
    FOREIGN KEY (`experiment_platform`)
    REFERENCES `crowdcontrol`.`Experiments_Platform` (`idExperiments_Platforms`)
    ON DELETE CASCADE
    ON UPDATE CASCADE)
ENGINE = InnoDB
...
CREATE TABLE IF NOT EXISTS `crowdcontrol`.`Experiments_Platform_Status` (
  `idExperiments_Platform_Status` INT NOT NULL,
  `status` ENUM('draft', 'running', 'finished', 'stopping', 'stopped') NOT NULL DEFAULT 'draft',
  `timestamp` TIMESTAMP NOT NULL DEFAULT CURRENT_TIMESTAMP,
...
CREATE TABLE IF NOT EXISTS `crowdcontrol`.`Experiments_Platform_Mode` (
  `idExperiments_Platform_Stopgap` INT NOT NULL,
  `experiments_platform` INT NOT NULL,
...
CREATE TABLE IF NOT EXISTS `crowdcontrol`.`Database_Version` (
  `idDatabase_Version` INT NOT NULL,
